...
for f in "${dump_dir}"/*_dump.txt; do
  ...
  ' "$f")
  #round to no after dot,nearest integer.  since the count of HTF is integer
  cutoff=$(echo "$stats" | cut -f1 | awk '{printf "%.0f", $1}')
  mean_minus_sd=$(echo "$stats" | cut -f2 | awk '{printf "%.0f", $1}')
  mean_plus_sd=$(echo "$stats" | cut -f3 | awk '{printf "%.0f", $1}')
  mean_minus_0_5sd=$(echo "$stats" | cut -f4 | awk '{printf "%.0f", $1}')
  mean_plus_0_5sd=$(echo "$stats" | cut -f5 | awk '{printf "%.0f", $1}')
