...
# ---- Order: non-p first, then lowercase-p ----
non_p_files <- dump_files[!grepl("^PL", basename(dump_files))]
p_files     <- dump_files[ grepl("^PL", basename(dump_files))]
dump_files  <- c(p_files, non_p_files)
...
p_filt <- ggplot(freq_df_filt, aes(x = count, y = frequency)) +
  ...
  labs(
    title = paste("Filtered (count >2, <= mean+3sd):", sample),
    subtitle = sprintf("mean=%.2f, sd=%.2f, mean+3sd=%.2f",
