...
# Modern samples → 1 red dashed line (cutoff)
# Historical samples → 2 red dashed lines (mean ± SD)
# =============================================================
...
out_dir <- "/SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/results/step1_HTFhaplotypes/step1.2_HTF_bykmers/step3_query_kmers/kmer_distribution_mix/distributions_1_5sd"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# --- Thresholds ---
threshold_modern <- fread(file.path(wg_dir, "threshold_m57.tsv"), col.names = c("sample", "cutoff"))
threshold_historical <- fread(file.path(wg_dir, "threshold_h49_sd_twosides_1.5sd.tsv"), col.names = c("sample", "mean_minus_1.5sd", "mean_plus_1.5sd"))
threshold_map_modern <- setNames(threshold_modern$cutoff, threshold_modern$sample)
threshold_map_historical_low <- setNames(threshold_historical$mean_minus_1.5sd, threshold_historical$sample)
threshold_map_historical_high <- setNames(threshold_historical$mean_plus_1.5sd, threshold_historical$sample)
...
# Append sample name to log file
  write(sample_name, file = file.path(out_dir, "noHTF.txt"), append = TRUE)
  quit(status = 0)
...
if (is_modern) {
  ...
  p <- p +
    ...
    geom_vline(xintercept = threshold_high, color = "red", linetype = "dashed", linewidth = 0.8)
}
