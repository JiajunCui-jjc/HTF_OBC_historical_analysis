...
out_plot <- "/Users/cuijiajun/Desktop/others/tmphernan/2025_summerpaper_all/2025_summer_paperfig_m57/results/step1_HTFhaplotypes/step1.2_HTF_bykmers/step4_additionalanalysis/step3_HTFfreq/step3_h38_m57_OTU5candidates_map_HTFhaplotypes_modernTransparent.pdf"
...
p <- ggplot() +
  ...
  position = position_jitter(width = 2, height = 2),  # wider spread for Germany
  size = 2, alpha = 0.1
  ) +
  ...
  aes(x = lon, y = lat, color = LengthGroup),
  position = position_jitter(width = 2.2, height = 3),
  size = 2, alpha = 1
  ) +
...
# === New MAP with triangles for Modern ===
p_triangles <- ggplot() +
  geom_sf(data = world, fill = "#F5F5F5", color = "#CCCCCC", size = 0.1) +
  
  # Germany (Modern = triangles, slightly transparent just for density)
  geom_point(
    data = filter(samples_plot, country == "Germany" & IsolateType == "Modern"),
    aes(x = lon, y = lat, color = LengthGroup, shape = IsolateType),
    position = position_jitter(width = 2.2, height = 2),
    size = 1.5, alpha = 1
  ) +
  
  # Spain + Germany Historical = circles
  geom_point(
    data = filter(samples_plot, country %in% c("Spain", "Germany") & IsolateType == "Historical"),
    aes(x = lon, y = lat, color = LengthGroup, shape = IsolateType),
    position = position_jitter(width = 2.2, height = 3),
  ...
  scale_color_manual(values = length_colors) +
  scale_shape_manual(values = c("Historical" = 16, "Modern" = 17)) + # circle vs triangle
  coord_sf(xlim = c(-15, 55), ylim = c(30, 70), expand = FALSE) +
  theme_minimal(base_size = 14) +
  theme(
    panel.grid = element_blank(),
...
ggsave(
  "/Users/cuijiajun/Desktop/others/tmphernan/2025_summerpaper_all/2025_summer_paperfig_m57/results/step1_HTFhaplotypes/step1.2_HTF_bykmers/step4_additionalanalysis/step3_HTFfreq/step3_h38_m57_OTU5candidates_map_HTFhaplotypes_triangles.pdf",
  p_triangles, width = 10, height = 6
)
cat("✅ Saved triangle map\n")

# === Legend-only with triangle for Modern ===
legend_df <- expand.grid(
  LengthGroup = length_order,
  IsolateType = c("Historical", "Modern")
) %>%
  mutate(
    LengthGroup = factor(LengthGroup, levels = rev(length_order)),
    IsolateType = factor(IsolateType, levels = c("Historical", "Modern")),
    x = as.numeric(IsolateType),
    y = as.numeric(LengthGroup)
  )

p_legend_tri <- ggplot(legend_df, aes(x = x, y = y, color = LengthGroup, shape = IsolateType)) +
  geom_point(size = 6) +
  scale_color_manual(values = length_colors, name = "HTF haplotypes (bp)") +
  scale_shape_manual(values = c("Historical" = 16, "Modern" = 17), name = "Sample type") +
  scale_x_continuous(
    breaks = c(1, 2),
    labels = c("Historical", "Modern"),
    expand = expansion(mult = c(0.2, 0.2))
...
ggsave(
  "/Users/cuijiajun/Desktop/others/tmphernan/2025_summerpaper_all/2025_summer_paperfig_m57/results/step1_HTFhaplotypes/step1.2_HTF_bykmers/step4_additionalanalysis/step3_HTFfreq/legend_only_triangles.pdf",
  p_legend_tri, width = 4, height = 2.5
)
cat("✅ Saved triangle legend\n")