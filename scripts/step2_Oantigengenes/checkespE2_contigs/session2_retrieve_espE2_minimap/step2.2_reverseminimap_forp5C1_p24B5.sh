...
MINIMAP_OUTPUT_DIR="${RESULT_DIR}/minimap_results_reverse"
...
while read -r sample; do
    ...
    if [[ -f "$fasta" ]]; then
        $MINIMAP_CMD -cx asm5 "$QUERY" "$fasta" > "$paf_file"
    else
...
input="sumpaf.txt"
ls -lh | grep ' 0 ' | cut -d ' ' -f 12 | cut -d '_' -f 1 > sub_absent_nomatch.txt
# Output files
full="sub_fulllengthmatched.txt"
partial="sub_retrievepaf.txt"
