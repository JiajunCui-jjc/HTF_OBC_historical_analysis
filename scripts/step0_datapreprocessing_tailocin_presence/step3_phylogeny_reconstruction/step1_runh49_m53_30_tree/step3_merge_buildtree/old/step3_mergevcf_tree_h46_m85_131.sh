...
# ============================================================
# Merge 131 VCFs (h46 + m55 + m30), filter SNPs, and build tree
# ============================================================
...
RESULTS_DIR="${OUT_TOP}/results_h46_m85_131"
...
VCF_M55="${OUT_TOP}/vcfs_m55"
VCF_M30="${OUT_TOP}/vcfs_m30"
VCF_COMBINED="${RESULTS_DIR}/vcfs_131_h46_m85"
...
# ============================================================
echo "[INFO] Linking all 131 VCFs..."
cd "${VCF_COMBINED}"

ln -sf ${VCF_H46}/*.ploidy1_filtered.sorted.vcf.gz* .
ln -sf ${VCF_M55}/*.ploidy1_filtered.sorted.vcf.gz* .
...
/SAN/ugi/plant_genom/software/bcftools-1.11/bcftools merge \
    ...
    -Oz -o merged_h46_m85_131samples.vcf.gz

/SAN/ugi/plant_genom/software/bcftools-1.11/bcftools index -t merged_h46_m85_131samples.vcf.gz
...
nofall=$(bcftools view -g ^miss -H merged_h46_m85_131samples.vcf.gz | wc -l)

bcftools view -M2 merged_h46_m85_131samples.vcf.gz | bgzip > nomulti_h46_m85_131samples.vcf.gz
bcftools index -t nomulti_h46_m85_131samples.vcf.gz
nofnomulti=$(bcftools view -g ^miss -H nomulti_h46_m85_131samples.vcf.gz | wc -l)

bcftools view -m2 -M2 merged_h46_m85_131samples.vcf.gz | gzip > bialleliconly_h46_m85_131samples.vcf.gz
nofbiallelic=$(bcftools view -g ^miss -H bialleliconly_h46_m85_131samples.vcf.gz | wc -l)
...
python /SAN/ugi/plant_genom/jiajucui/phylogeny/phylogeny_snp/vcf2phylip/vcf2phylip.py \
    -i bialleliconly_h46_m85_131samples.vcf.gz -m 131

mv bialleliconly_h46_m85_131samples.min131.phy "${TREE_DIR}/"

cd "${TREE_DIR}"
echo "[INFO] Running IQ-TREE..."
iqtree -T AUTO \
    -s bialleliconly_h46_m85_131samples.min131.phy \
    -m MFP -B 1000 -alrt 1000 | tee iqtree_131.log
