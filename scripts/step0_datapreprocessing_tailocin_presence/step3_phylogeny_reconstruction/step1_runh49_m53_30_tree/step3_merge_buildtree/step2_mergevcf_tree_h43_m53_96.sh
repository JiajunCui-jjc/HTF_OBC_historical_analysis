...
#$ -cwd
#$ -V
#$ -N h43_m53_merge_tree
#$ -hold_jid h49_callvcf,m53_callvcf
#$ -o /SAN/ugi/plant_genom/jiajucui/logs/
#$ -e /SAN/ugi/plant_genom/jiajucui/logs/
#$ -hold_jid h49_callvcf,m30_callvcf,m53_callvcf

# ============================================================
# Merge h43 (ATUE5-only historical) + m53 (modern ATUE5)
...
RESULTS_DIR="${OUT_TOP}/results_h43_m53"
...
VCF_COMBINED="${RESULTS_DIR}/vcfs_h43_m53"
mkdir -p "${VCF_COMBINED}"

# ---- Historical samples to exclude ----
EXCLUDE_SAMPLES=("HB0828" "HB0863" "PL0066" "PL0108" "PL0203" "PL0258")
#BED="/SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/scripts/step0_datapreprocessing_tailocin_presence/step3_phylogeny_reconstruction/step1_runh49_m53_30_tree/step3_merge_buildtree/exclude_HTF_TFA_auto.bed"
...
# Link historical 43 (filter out non-ATUE5)
for f in ${VCF_H49}/*.ploidy1_filtered.sorted.vcf.gz; do
...
# ============================================================
echo "[INFO] Merging h43 + m53 VCFs..."
/SAN/ugi/plant_genom/software/bcftools-1.11/bcftools merge \
    ${VCF_COMBINED}/*.ploidy1_filtered.sorted.vcf.gz \
    -Oz -o merged_h43_m53_96samples.vcf.gz

/SAN/ugi/plant_genom/software/bcftools-1.11/bcftools index -t merged_h43_m53_96samples.vcf.gz
...
nofall=$(bcftools view -g ^miss -H merged_h43_m53_96samples.vcf.gz | wc -l)

bcftools view -M2 merged_h43_m53_96samples.vcf.gz | bgzip > nomulti_h43_m53_96samples.vcf.gz
bcftools index -t nomulti_h43_m53_96samples.vcf.gz
nofnomulti=$(bcftools view -g ^miss -H nomulti_h43_m53_96samples.vcf.gz | wc -l)

bcftools view -m2 -M2 merged_h43_m53_96samples.vcf.gz | gzip > bialleliconly_h43_m53_96samples.vcf.gz
nofbiallelic=$(bcftools view -g ^miss -H bialleliconly_h43_m53_96samples.vcf.gz | wc -l)
...
python /SAN/ugi/plant_genom/jiajucui/phylogeny/phylogeny_snp/vcf2phylip/vcf2phylip.py \
    -i bialleliconly_h43_m53_96samples.vcf.gz -m 96

mv bialleliconly_h43_m53_96samples.min96.phy "${TREE_DIR}/"

cd "${TREE_DIR}"
echo "[INFO] Running IQ-TREE..."
iqtree -T AUTO \
    -s bialleliconly_h43_m53_96samples.min96.phy \
    -m MFP -B 1000 -alrt 1000 | tee iqtree_96.log
