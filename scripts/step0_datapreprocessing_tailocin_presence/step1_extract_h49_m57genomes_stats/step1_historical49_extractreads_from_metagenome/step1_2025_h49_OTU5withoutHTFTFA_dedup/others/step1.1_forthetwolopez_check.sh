...
# ===========================================================
i=${SGE_TASK_ID}
pathtmp=2025_maptoOTU5_nohaplotype_h49/h2_lopez
...
answerspath=/SAN/ugi/plant_genom/jiajucui/answer2025_maptoOTU5_nohaplotype_h49/h10_lopez

mkdir -p "$basePs" "$baseAt" "$answerspath"

sample_list=/SAN/ugi/plant_genom/jiajucui/1_initial_data/new_sequences/h2.txt
...
raw=/SAN/ugi/plant_genom/jiajucui/2_trimmed_merged/${pathtmp}/
...
# ===========================================================
# 4. Pseudomonas stats (ref without HTF/TFA)
# ===========================================================
...
# --- pre-dedup read count (mapping percentage) ---
samtools flagstat "$in_bam" > "${basePs}/${samplename}_Ps_q20_flagstats.log"
echo "q2 What is the percentage of Pseudomonas DNA after removing mapped reads to A.thaliana?" >> "$out_answer"
grep 'mapped (' "${basePs}/${samplename}_Ps_q20_flagstats.log" >> "$out_answer"

# --- dedup (sort → rmdup → sort + index) ---
...
# --- coverage and depth (after dedup) ---
covfile="${basePs}/${samplename}.mapped_to_Pseudomonas.dd.q20.rmdup.coverage.txt"
samtools depth -aa "$dedup_bam" > "$covfile"
...
# --- NM distribution (mismatch frequency) ---
...
# --- cleanup ---
rm -f "$covfile" "${in_bam%.bam}.sorted.bam"
# remove intermediate and temporary files
rm -f ${basePs}/${samplename}.collapsed_Ps.sai \
      ${basePs}/${samplename}.R1.sai \
      ${basePs}/${samplename}.R2.sai \
      ${basePs}/${samplename}.collapsed_unmapped.bam \
      ${basePs}/${samplename}.r1r2_unmapped.bam \
      ${basePs}/${samplename}.r1r2_unmapped.R1.fq.gz \
      ${basePs}/${samplename}.r1r2_unmapped.R2.fq.gz \
      ${basePs}/${samplename}_Ps_q20_flagstats.log \
      ${basePs}/${samplename}_removalAt_mapped_to_ps.bam \
      ${basePs}/${samplename}.unsort_mapped_to_Pseudomonas.dd.q20.bam \
      ${basePs}/${samplename}.collapsed_Ps.q20.bam \
      ${basePs}/${samplename}.collapsed_Ps.q20.bam.bai \
      ${basePs}/${samplename}.collapsed_Ps.sorted.bam \
      ${basePs}/${samplename}.r1r2_Ps.q20.bam \
      ${basePs}/${samplename}.r1r2_Ps.q20.bam.bai \
      ${basePs}/${samplename}.r1r2_Ps.sorted.bam
rm $baseAt/${samplename}.collapsed.sai

# ===========================================================
# 5. Symlink deduplicated BAMs (Lopez10 subset under h49 structure)
# ===========================================================
echo "Linking deduplicated BAMs for Lopez10 subset into h49 structure..."

At_linkdir=/SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/data/all_fastq_h49/At_bams_maptoOTU5withouthaplotype_h49_dedup/h10_lopez
Ps_linkdir=/SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/data/all_fastq_h49/Ps_bams_maptoOTU5withouthaplotype_h49_dedup/h10_lopez
mkdir -p "$At_linkdir" "$Ps_linkdir"

ln -sf ${baseAt}/${samplename}_mapped_At_q20.rmdup.sorted.bam* "$At_linkdir/"
ln -sf ${basePs}/${samplename}.mapped_to_Pseudomonas.dd.q20.rmdup.bam* "$Ps_linkdir/"

# ===========================================================
# 6. Cleanup A.thaliana intermediate files
# ===========================================================
find "$baseAt" -type f \( \
