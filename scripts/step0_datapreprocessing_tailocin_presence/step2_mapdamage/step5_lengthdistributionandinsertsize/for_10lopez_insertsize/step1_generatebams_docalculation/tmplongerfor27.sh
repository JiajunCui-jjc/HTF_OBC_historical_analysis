...
#$ -S /bin/bash
#$ -N tmp27_2025_10
#$ -o /SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/results/step0_datapreprocessing_tailocin_presence/step1_h40metadata/step2_lengthdist_git_andnsqs10insertsize/logs/
#$ -e /SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/results/step0_datapreprocessing_tailocin_presence/step1_h40metadata/step2_lengthdist_git_andnsqs10insertsize/logs/
#$ -wd /SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/results/step0_datapreprocessing_tailocin_presence/step1_h40metadata/step2_lengthdist_git_andnsqs10insertsize/
...
base_out=/SAN/ugi/plant_genom/jiajucui/4_mapping_to_pseudomonas/tailocin_2024_TF_Tapemeasure/2025_summer_paperfig_m57/results/step0_datapreprocessing_tailocin_presence/step1_h40metadata/step2_lengthdist_git_andnsqs10insertsize
...
mkdir -p "$trimdir" "$outdir_qc" "$outdir_At" "$outdir_Ps" "$outdir_prop" "$outdir_ins"
...
#samtools index ${outdir_At}/${samplename}.collapsed_At.q20.bam
...
samtools index ${outdir_At}/${samplename}.r1r2_At.q20.bam

# Clean At intermediates
rm -f ${outdir_At}/${samplename}.collapsed.sai \
...
| samtools sort -o ${outdir_Ps}/${samplename}.r1r2_Ps.q20.bam

samtools index ${outdir_Ps}/${samplename}.r1r2_Ps.q20.bam
...
samtools index ${outdir_Ps}/${samplename}.collapsed_Ps.q20.bam
...
samtools view ${outdir_At}/${samplename}.collapsed_At.q20.bam | cut -f 10 \
...
samtools view -f 66 ${outdir_At}/${samplename}.r1r2_At.q20.bam | cut -f 9 \
...
samtools view ${outdir_Ps}/${samplename}.collapsed_Ps.q20.bam | cut -f 10 \
  | awk '{print length($0)}' | sort -n | uniq -c \
...
samtools view -f 66 ${outdir_Ps}/${samplename}.r1r2_Ps.q20.bam | cut -f 9 \
  | awk '{if($1<0) $1=-$1} {c[$1]++} END{for (k in c) print k"\t"c[k]}' \
  | sort -k1,1n \
...
meanAt=$(samtools view -f 66 ${outdir_At}/${samplename}.r1r2_At.q20.bam | cut -f 9 \
         | awk '{if($1<0) $1=-$1; sum+=$1} END{if(NR>0) print sum/NR; else print 0}')
meanPs=$(samtools view -f 66 ${outdir_Ps}/${samplename}.r1r2_Ps.q20.bam | cut -f 9 \
         | awk '{if($1<0) $1=-$1; sum+=$1} END{if(NR>0) print sum/NR; else print 0}')
