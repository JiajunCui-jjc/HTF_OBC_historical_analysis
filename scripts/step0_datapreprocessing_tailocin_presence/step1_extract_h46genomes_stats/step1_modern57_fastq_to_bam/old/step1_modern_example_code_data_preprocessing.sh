...
bwa aln -t 2 -l 1024 -f ${wd}/all_bams_m57//${samplename}.r1.collapsed.sai ${refPs} ${r1} &&
echo 'bwa aln r1 done' &&
bwa aln -t 2 -l 1024 -f ${wd}/all_bams_m57//${samplename}.r2.collapsed.sai ${refPs} ${r2} &&
echo 'bwa aln r2 done' &&
#Convert  reads into a standard alignment format (SAM)
bwa sampe -r @RG\\tID:${samplename}\\tSM:${samplename} -f ${wd}/all_bams_m57//${samplename}.collapsed_Ps.sam ${refPs} ${wd}/all_bams_m57//${samplename}.r1.collapsed.sai ${wd}/all_bams_m57//${samplename}.r2.collapsed.sai ${r1} ${r2} &&
echo 'bwa sampe done' &&

#convert mapped reads into bam with map quality score greater than 80 
samtools view -@ 2 -F 4  -q 20 -Sbh -o ${wd}/all_bams_m57//${samplename}_mapped_Ps_q20unsort.bam ${wd}/all_bams_m57//${samplename}.collapsed_Ps.sam

samtools sort -@ 2 -o ${wd}/all_bams_m57//${samplename}_mapped_Ps_q20.bam ${wd}/all_bams_m57//${samplename}_mapped_Ps_q20unsort.bam
